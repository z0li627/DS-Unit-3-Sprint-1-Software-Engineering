...
Feedback to the coder. Asking the coder about her/his code. Constructive comments.

Readability, transparency, comments in the code.


Containers are like virtual machines. With containers we can standardize the
environment. We can set up all containers with the same configuration.
If we have a configuration file, it takes just a couple minutes to set up a
new container. Also less resource needed than for a virtual machine.
